...
impl Simulation {
    pub(in crate::simulation) fn normalize_player_ship_roster(&mut self) {
        let mut player_candidates = self
            .ships
            .iter()
            .filter(|(_, ship)| ship.company_id == CompanyId(0) || ship.role == ShipRole::Player)
        ...
        if player_candidates.is_empty() {
            ...
            let (descriptor, modules, technical_state) =
                super::stage_a_ship_metadata(ship_id, CompanyId(0), ShipRole::Player);
            self.ships.insert(
                ship_id,
                Ship {
                    id: ship_id,
        ...
        for ship in self.ships.values_mut() {
            if ship.id != keep_player_ship
                && (ship.company_id == CompanyId(0) || ship.role == ShipRole::Player)
            {
            ...
            if let Some(mission) = self.missions.get(mission_id) {
                if !matches!(
                    ...
                    queue.push(FleetJobStep {
                        ...
                        system: mission.destination,
                        eta_ticks: eta_cursor,
                    });
                }
            }
        }
        if mission_ids.is_empty() && ship.trade_order_id.is_none() {
